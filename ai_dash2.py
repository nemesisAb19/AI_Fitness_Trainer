...
def launch_gui():
    ...
    def handle_button_click(name):
        if name == "Pushups":
            ...
            subprocess.Popen(["python", "bicepcurl_page_ai.py"])
            root.destroy()
        elif name == "Mountainclimbers":
            subprocess.Popen(["python", "mountainclimbers_page_ai.py"])
            root.destroy()
        elif name == "Shoulder":
            subprocess.Popen(["python", "shoulderpress_page_ai.py"])
            root.destroy()
        elif name == "Squat":
            subprocess.Popen(["python", "squat_page_ai.py"])
            root.destroy()
        elif name == "Pullups":
            subprocess.Popen(["python", "pullups_page_ai.py"])
            root.destroy()
        elif name == "Benchpress":
            subprocess.Popen(["python", "benchpress_page_ai.py"])
            root.destroy()
        elif name == "Situps":
            subprocess.Popen(["python", "crunches_page_ai.py"])
            root.destroy()
        # elif name == "Workouts":
