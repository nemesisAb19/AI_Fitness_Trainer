...
def Weight_Loss():
    ...
    # Food_items, Breakfast, Lunch, Dinner, VegNovVeg, Calories, Fats, etc.
    Food_itemsdata = data["Food_items"]

    # Get the meal indicator columns.
    Breakfastdata = data["Breakfast"]
    Lunchdata = data["Lunch"]
    Dinnerdata = data["Dinner"]

    # Partition food items by meal based on these columns.
    breakfastfoodseparated = []
    Lunchfoodseparated = []
    Dinnerfoodseparated = []

    # Create lists to store the row indices for each meal.
    ...
    # (Optional) Display the meals.
    print("\n[MEALS]")
    print("  Breakfast:", breakfastfoodseparated)
    print("  Lunch    :", Lunchfoodseparated)
    print("  Dinner   :", Dinnerfoodseparated)

    # For processing, we extract a subset of columns from the CSV.
    val = list(np.arange(5, 15))
    Valapnd = [0] + val  # Keep the Food_items column (index 0) plus others.
    
    # Retrieve data rows corresponding to meal indices.
    ...
    age = int(e1.get())
    veg = float(e2.get())   # 1 if user is veg; 0 if nonveg.
    weight = float(e3.get())
    height = float(e4.get())
    ...
    # Set the BMI classification flag (used later in clustering/training).
    if bmi < 16:
        clbmi = 4
    elif bmi < 18.5:
        clbmi = 3
    ...
    # Convert the meal data DataFrames into numpy arrays.
    ...
    # Run KMeans clustering separately on each meal.
    ...
    for ii in range(max_items):
        if y_pred[ii] == 2:
            ...
            # Use the CSV's VegNovVeg value for filtering.
            # For a veg user (veg == 1), only include food items where VegNovVeg == 0.
            if int(veg) == 1 and int(data["VegNovVeg"].iloc[ii]) != 0:
                print("Skipping non-veg item:", food_item)
                continue
            # Partition the recommendation into meal categories.
...
def Weight_Gain():
    ...
    for ii in range(max_items):
        if y_pred[ii] == 2:
            food_item = Food_itemsdata.iloc[ii]
            # Filter using the VegNovVeg column.
            if int(veg) == 1 and int(data["VegNovVeg"].iloc[ii]) != 0:
                print("Skipping non-veg item:", food_item)
                continue
            if ii in breakfastfoodseparatedID:
