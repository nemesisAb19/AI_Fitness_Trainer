...
def launch_gui():
    ...
    button_data = [
        ("exercise_button.jpeg", 304, 120),
        ("build_button.jpeg", 304, 250),
    ...
    def handle_button_click(name):
        if name == "Exercise":
            subprocess.Popen(["python", "ai_dash2.py"])
            root.destroy()
        elif name == "Build":
    ...
    for img_path, x, y in button_data:
        ...
        normal_img = Image.open(img_path).resize((350, 135), Image.LANCZOS).convert("RGBA")
