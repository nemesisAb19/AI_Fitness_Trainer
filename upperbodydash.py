...
def launch_gui():
    ...
    button_data = [
        ("pushups_button.jpeg", 110, 55),
        ("bicepcurls_button.jpeg", 360, 55),
        ("mountainclimbers_button.jpeg", 610, 55),
        ("shoulder_button.jpeg", 110, 270),
        ("benchpress_button.jpeg", 360, 270),
        ("pullups_button.jpeg", 610, 270),
        # ("workouts_button.jpeg", 445, 250),
    ]

    def handle_button_click(name):
        if name == "Pushups":
            subprocess.Popen(["python", "pushups_page.py"])
            root.destroy()
        elif name == "Bicepcurls":
            subprocess.Popen(["python", "bicepcurl_page.py"])
            root.destroy()
        elif name == "Mountainclimbers":
            subprocess.Popen(["python", "mountainclimbers_page.py"])
            root.destroy()
        elif name == "Shoulder":
            subprocess.Popen(["python", "shoulderpress_page.py"])
            root.destroy()
        elif name == "Benchpress":
            subprocess.Popen(["python", "benchpress_page.py"])
            root.destroy()
        elif name == "Pullups":
            subprocess.Popen(["python", "pullups_page.py"])
            root.destroy()
        # elif name == "Workouts":
    ...
    for img_path, x, y in button_data:
        ...
        normal_img = Image.open(img_path).resize((200, 200), Image.LANCZOS).convert("RGBA")
    ...
    root,
    text="Previous",
    command=lambda: (root.destroy(), subprocess.Popen(["python", "no_ai.py"])),
    bg="#ecc474",
    fg="#000000",
    font=("Helvetica", 6, "bold"),
    bd=4,
